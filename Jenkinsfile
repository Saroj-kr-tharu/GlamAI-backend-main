    ...
    stages{
        ...
        stage(" Build Docker Image "){ 
          steps{
             ...
             withCredentials(  [usernamePassword(
                        ...
                        sh '''
                          docker build -t ${dockerHubUser}/glamai-backend:latest .
                        '''
        ...
        stage("Docker Image Scan") {
          steps {
              withCredentials([usernamePassword(
                  ...
                  sh '''
                    trivy image --format json -o backend-image-scan.json ${dockerHubUser}/glamai-backend:latest
            ...
            steps{
                ...
                withCredentials(  [usernamePassword(
                        ...
                        sh '''
                          ...
                          docker push ${dockerHubUser}/glamai-backend:latest
        ...
        stage("k8s Deployment Restart"){ 
          steps{
             ...
             sh "kubectl rollout restart deployment backend-dep -n glamai-ns"
         } }
    }
  ...
  post {
    success {
        script {
            ...
            emailext(
                mimeType: 'text/html',
                attachmentsPattern: 'result.json, backend-image-scan.json,  dependency-check-report/dependency-check-report.xml',
